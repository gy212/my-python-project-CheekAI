...
let currentDetectData = null
let currentViewMode = 'paragraph' // 'paragraph', 'sentence', 'comparison'
...
function renderResult(data) {
  ...
  // Render view mode tabs if dual detection is available
  if (data.dualDetection) {
    renderViewModeTabs()
  }

  // Render segments based on current view mode
  renderSegmentsForCurrentMode()

}

function renderViewModeTabs() {
  const tabsHtml = `
    <div class="view-mode-tabs" style="margin-bottom: 16px; display: flex; gap: 8px; border-bottom: 2px solid #e0e0e0;">
      <button class="view-tab ${currentViewMode === 'paragraph' ? 'active' : ''}" onclick="switchViewMode('paragraph')">
        分段视图
      </button>
      <button class="view-tab ${currentViewMode === 'sentence' ? 'active' : ''}" onclick="switchViewMode('sentence')">
        分句视图
      </button>
      <button class="view-tab ${currentViewMode === 'comparison' ? 'active' : ''}" onclick="switchViewMode('comparison')">
        对比视图
      </button>
    </div>
  `
  segmentsDiv.insertAdjacentHTML('beforebegin', tabsHtml)
}

function switchViewMode(mode) {
  currentViewMode = mode
  if (currentDetectData) {
    // Remove old tabs
    const oldTabs = document.querySelector('.view-mode-tabs')
...
function renderSegmentsForCurrentMode() {
  if (!currentDetectData) return
  
  segmentsDiv.innerHTML = ''
  
  if (currentViewMode === 'paragraph') {
    renderSegments(currentDetectData.segments, '段')
  } else if (currentViewMode === 'sentence' && currentDetectData.dualDetection) {
    renderSegments(currentDetectData.dualDetection.sentence.segments, '句块')
  } else if (currentViewMode === 'comparison' && currentDetectData.dualDetection) {
    renderComparisonView()
  } else {
    // Fallback to paragraph view
    renderSegments(currentDetectData.segments, '段')
  }
}

function renderSegments(segments, labelPrefix) {
  for (const s of segments) {
    ...
    card.innerHTML = `
      ...
      <div class="card-header segment-header">${labelPrefix} ${s.chunkId}</div>
...
function renderComparisonView() {
  const dual = currentDetectData.dualDetection
  if (!dual) return
  
  const container = document.createElement('div')
  container.style.cssText = 'display: grid; grid-template-columns: 1fr 1fr; gap: 16px;'
  
  // Left: Paragraph results
  const leftDiv = document.createElement('div')
  leftDiv.innerHTML = '<h3 style="margin-bottom: 12px;">分段检测</h3>'
  leftDiv.style.cssText = 'border-right: 2px solid #e0e0e0; padding-right: 12px;'
  
  const paraContainer = document.createElement('div')
  for (const s of dual.paragraph.segments) {
    const card = createSegmentCard(s, '段', dual.comparison.divergentRegions)
    paraContainer.appendChild(card)
  }
  leftDiv.appendChild(paraContainer)
  
  // Right: Sentence results
  const rightDiv = document.createElement('div')
  rightDiv.innerHTML = '<h3 style="margin-bottom: 12px;">分句检测</h3>'
  rightDiv.style.cssText = 'padding-left: 12px;'
  
  const sentContainer = document.createElement('div')
  for (const s of dual.sentence.segments) {
    const card = createSegmentCard(s, '句块', dual.comparison.divergentRegions)
    sentContainer.appendChild(card)
  }
  rightDiv.appendChild(sentContainer)
  ...
  statsDiv.style.cssText = 'grid-column: 1 / -1; padding: 16px; background: #f5f5f5; border-radius: 8px; margin-top: 16px;'
  const comparison = dual.comparison
  statsDiv.innerHTML = `
    <h4>对比分析</h4>
    <div style="display: flex; gap: 24px; margin-top: 8px;">
      <div>概率差异: <strong>${(comparison.probabilityDiff * 100).toFixed(1)}%</strong></div>
      <div>一致性评分: <strong>${(comparison.consistencyScore * 100).toFixed(0)}%</strong></div>
      <div>风险点数量: <strong>${comparison.divergentRegions.length}</strong></div>
    </div>
  `
...
function createSegmentCard(s, labelPrefix, divergentRegions) {
  const card = document.createElement('div')
  card.className = 'card segment-card'
  ...
  // Check if this segment is in divergent regions
  const isDivergent = divergentRegions.some(r => 
    r.paragraphSegmentId === s.chunkId || r.sentenceSegmentId === s.chunkId
  )
  ...
  card.innerHTML = `
    <div class="card-header segment-header">${labelPrefix} ${s.chunkId}</div>
    <div class="metrics">
      <div class="metric"><span class="metric-label">AI 概率</span><span class="metric-value">${ai}%</span></div>
      <div class="metric"><span class="metric-label">置信度</span><span class="metric-value">${conf}%</span></div>
      <div class="metric"><span class="metric-label">词汇多样性</span><span class="metric-value">${ttr}</span></div>
    </div>
    ${isDivergent ? '<div style="color: #ff9800; font-size: 12px; margin-top: 4px;">⚠️ 差异区域</div>' : ''}
  `
...
// Make switchViewMode globally available
...
async function batchDetect(files) {
  ...
  const preprocessTasks = Array.from(files).map(async f => {
    ...
    if (!rsp.ok) {
      ...
      throw new Error(`棰勫鐞嗘枃浠?${f.name} 澶辫触锛TTP ${rsp.status}`)
