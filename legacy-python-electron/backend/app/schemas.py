...
class ModeDetectionResult(BaseModel):
    """Detection result for a single mode (paragraph or sentence)"""
    aggregation: AggregationResponse
    segments: List[SegmentResponse]
    segmentCount: int


class DivergentRegion(BaseModel):
    """Region where paragraph and sentence modes have significant difference"""
    paragraphSegmentId: int
    sentenceSegmentId: int
...
class ComparisonResult(BaseModel):
    """Comparison between paragraph and sentence detection modes"""
    probabilityDiff: float
    consistencyScore: float
    divergentRegions: List[DivergentRegion] = Field(default_factory=list)


class DualDetectionResult(BaseModel):
    """Result containing both paragraph and sentence detection"""
...
class DetectResponse(BaseModel):
    ...
    version: str
    requestId: str
    dualDetection: Optional[DualDetectionResult] = None
