...
def splitSentencesAdvanced(text: str) -> List[Dict[str, Any]]:
    ...
    while i < len(text):
        ...
        if ch in '。\uff01\uff1f.!?':
            ...
            # Check for decimal numbers (e.g., 1.5)
            if ch == '.':
                # Look back and forward for digits
                ...
                if i > 2:
                    prev_3 = text[max(0, i-3):i+1].lower()
                    if prev_3 in ['dr.', 'mr.', 'ms.']:
                        i += 1
                        continue
                    if text[max(0, i-4):i+1].lower() in ['mrs.', 'etc.']:
                        i += 1
                        continue
            ...
            if ch == '…' and i < len(text) - 1 and text[i+1] == '…':
                buffer += text[i+1]
                i += 2
                # Check if followed by quote or newline
                if i < len(text) and text[i] in quote_chars + '\n':
...
def buildSentenceBlocks(
    ...
    """
    Build sentence-based detection blocks with adaptive aggregation.
    
    Args:
        text: Input text
        min_chars: Minimum chars for a standalone sentence
        target_chars: Target chars per block
        max_chars: Maximum chars before forcing a split
    
    Returns:
        List of blocks similar to paragraph blocks structure
    """
    ...
    current_block_sentences: List[Dict[str, Any]] = []
    current_block_chars = 0
    
    for sent in sentences:
        sent_len = len(sent["text"])
        
        # If sentence is very long, make it a standalone block
        if sent_len > max_chars:
            ...
            if current_block_sentences:
                ...
                blocks.append({
                    ...
                    "needDetect": True,
                    "mergeWithPrev": False,
                    "start": current_block_sentences[0]["start"],
                    "end": current_block_sentences[-1]["end"],
                    "text": block_text,
                    "sentenceCount": len(current_block_sentences)
                })
                current_block_sentences = []
                current_block_chars = 0
            
            # Add long sentence as standalone block
            blocks.append({
                "index": len(blocks),
                "label": "sentence_block",
                "needDetect": True,
        ...
        if current_block_chars + sent_len <= target_chars or current_block_chars == 0:
            current_block_sentences.append(sent)
            current_block_chars += sent_len
        else:
            ...
            # Start new block with current sentence
...
def compareDualModeResults(
    ...
    Args:
        paragraph_segments: Segments from paragraph mode
        sentence_segments: Segments from sentence mode
        text: Original text
        diff_threshold: Probability difference threshold for divergent regions
    
    Returns:
        Comparison result with probabilityDiff, consistencyScore, divergentRegions
    """
    if not paragraph_segments or not sentence_segments:
        return {
            "probabilityDiff": 0.0,
    ...
    para_probs = [s.get("aiProbability", 0.0) for s in paragraph_segments]
    sent_probs = [s.get("aiProbability", 0.0) for s in sentence_segments]
    
    para_avg = sum(para_probs) / max(1, len(para_probs))
    sent_avg = sum(sent_probs) / max(1, len(sent_probs))
    probability_diff = abs(para_avg - sent_avg)
    
    # Find divergent regions by matching overlapping segments
    divergent_regions: List[Dict[str, Any]] = []
    
    for p_seg in paragraph_segments:
        p_start = p_seg.get("offsets", {}).get("start", 0)
        p_end = p_seg.get("offsets", {}).get("end", 0)
        p_prob = p_seg.get("aiProbability", 0.0)
        
        # Find overlapping sentence segments
        for s_seg in sentence_segments:
            s_start = s_seg.get("offsets", {}).get("start", 0)
            s_end = s_seg.get("offsets", {}).get("end", 0)
            s_prob = s_seg.get("aiProbability", 0.0)
            ...
            if overlap_len > 0:
                ...
                p_coverage = overlap_len / max(1, p_end - p_start)
                s_coverage = overlap_len / max(1, s_end - s_start)
                
                # If significant overlap (>50%), compare probabilities
                if p_coverage > 0.5 and s_coverage > 0.5:
                    prob_diff = abs(p_prob - s_prob)
                    
                    if prob_diff > diff_threshold:
                        # Extract text preview
                        preview_start = overlap_start
                        preview_end = min(overlap_start + 100, overlap_end)
                        text_preview = text[preview_start:preview_end]
                        if preview_end < overlap_end:
                            text_preview += "..."
                        
                        divergent_regions.append({
                            "paragraphSegmentId": p_seg.get("chunkId", -1),
                            "sentenceSegmentId": s_seg.get("chunkId", -1),
                            "probabilityDiff": round(prob_diff, 4),
                            "paragraphProb": round(p_prob, 4),
                            "sentenceProb": round(s_prob, 4),
                            "textPreview": text_preview
                        })
    
    # Calculate consistency score
    # Count how many segment pairs agree on direction (both >0.5 or both <=0.5)
    consistent_count = 0
    total_comparisons = 0
    ...
    for p_seg in paragraph_segments:
        ...
        p_end = p_seg.get("offsets", {}).get("end", 0)
        p_prob = p_seg.get("aiProbability", 0.0)
        p_direction = 1 if p_prob > 0.5 else 0
        
        for s_seg in sentence_segments:
            s_start = s_seg.get("offsets", {}).get("start", 0)
            s_end = s_seg.get("offsets", {}).get("end", 0)
            s_prob = s_seg.get("aiProbability", 0.0)
            s_direction = 1 if s_prob > 0.5 else 0
    ...
    consistency_score = consistent_count / max(1, total_comparisons) if total_comparisons > 0 else 1.0
    
    return {
        "probabilityDiff": round(probability_diff, 4),
...
async def detect_async(
    ...
    useStylometry: bool = True,
    sensitivity: str = "medium",
    enable_dual_detection: bool = True,
    ...
    # Dual detection: run sentence-based detection in parallel
    dual_detection_result = None
    if enable_dual_detection:
        try:
            # Build sentence blocks
            sentence_blocks = buildSentenceBlocks(normalized)
            
            # Run sentence-based segmentation
            sentence_segments = buildSegmentsAligned(
                normalized,
                language or "zh-CN",
                chunk_tokens,
                overlap_tokens,
            ...
            # Run LLM judgment for sentence segments if providers available
            if provider_specs:
                sentence_llm_stats = await _run_llm_judgment_v2(sentence_segments, provider_specs)
            
            _contrast_sharpen_segments(sentence_segments, sensitivity)
            sentence_agg = aggregateSegments(sentence_segments)
            sentence_agg["decision"] = deriveDecision(
                sentence_agg["overallProbability"],
                sentence_agg["thresholds"],
                DEFAULT_BUFFER_MARGIN
            )
            
            # Compare paragraph and sentence results
            ...
            dual_detection_result = {
                ...
                "sentence": {
                    "aggregation": sentence_agg,
                    "segments": sentence_segments,
                    "segmentCount": len(sentence_segments)
                },
                "comparison": comparison
            }
            
            logging.info(
                "dual_detection paragraph_segments=%s sentence_segments=%s prob_diff=%.4f consistency=%.4f",
                len(segments),
                len(sentence_segments),
                comparison.get("probabilityDiff", 0.0),
                comparison.get("consistencyScore", 0.0)
            )
        except Exception as e:
            logging.exception("dual_detection_failed error=%s", str(e))
            # If dual detection fails, continue with paragraph-only result
            dual_detection_result = None
    
    return agg, segments, preprocess_summary, cost, dual_detection_result
